...
# Calculate week 1 SD
o.sd <- oyster %>% 
  filter(date == as.Date('2025-06-19')) %>% 
  group_by(condition) %>% 
  summarise(sd.width = sd(width),
            sd.length = sd(length))
...
# Plot
ggplot(hobo.daily) +
  geom_line(aes(x=date, y=meanT, col=condition), lwd=1) +
...
buoy.for.plotting <- buoy.daily %>%
  filter(date >= as.Date('2025-06-19') & date<= as.Date('2025-08-07')) %>% 
