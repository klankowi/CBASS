...
lengths$period[lengths$year %in% c(2016, 2020, 2021, 2022, 2023, 2024, 2025)] <- 'hot'
...
cbs <- read.csv(here("Data/Clean_Data/Cohort/Cohort_Tracking_MTI_2025_2.csv"))
...
growth.est$period[growth.est$year %in% c(2016, 2020, 2021, 2022, 2023, 2024, 2025)] <- 'hot'
...
ggsave(plot=growthcomp,
       here('Documentation/MEPS/Figures/Growth Comparison 2025.png'),
...
for(i in 1:nrow(biolims)){
  print(
    ggplot(data=cohort.1[cohort.1$species_name == 
      ...
      geom_jitter(aes(x = week, y = length_mm),
                  alpha=0.25, width=0.2, stroke=NA, cex=2) +
      scale_color_viridis_d(option='viridis') +
...
ggplot(data=cohort.1[cohort.1$species_name == 'Silverside',]) +
    geom_jitter(aes(x = week, y = length_mm),
                alpha=0.25, width=0.2, stroke=NA, cex=2) +
    labs(color='Year', x='Week of year', y='Length (mm)', lty='Heat') +
    facet_wrap(vars(year)) +
...
save.image(file=here('Clean_Data/Cohort_Data_2025.RData'))
