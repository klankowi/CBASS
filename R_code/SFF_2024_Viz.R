...
ggplot(data=jpm.spawn) +
  geom_line(aes(x=date, y=daily.c)) +
  facet_wrap(vars(spawnyear), scales='free_x') +
  geom_ribbon(data=jpm.spawn,
              aes(x=date, ymin=rep(-1.1, nrow(jpm.spawn)),
                  ymax = rep(6.1), nrow(jpm.spawn)),
              alpha=0.1, fill='springgreen') +
  geom_ribbon(data=jpm.spawn,
              aes(x=date, ymin=rep(2.7, nrow(jpm.spawn)),
                  ymax = rep(12.6), nrow(jpm.spawn)),
              alpha=0.1, fill='blue') +
