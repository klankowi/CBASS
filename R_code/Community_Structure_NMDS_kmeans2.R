...
# Seven species make up more than 98% of total catch and are commonly caught
abund <- abund %>% 
  filter(species_name %in% comcat$species_name[35:41])
...
clus.char <- ggplot() + 
  ...
  stat_ellipse(data=data.scores,
               aes(x=NMDS1, y=NMDS2, fill=cluster),
               type='t', geom='polygon',
...
wholp <- ggpubr::ggarrange(a, b, c, d, 
                           e, 
                           nrow=5)
wholp
ggsave(plot=wholp,
