...
# Finalize length information - if field measurements lost, use lab
bio$length_mm <- bio$field_length_mm
bads <- bio[is.na(bio$length_mm),]
for(i in 1:nrow(bads)){
  if(!is.na(bads$lab_length_mm[i])){
    bads$length_mm[i] <- bads$lab_length_mm[i]
  }
}
bio <- bio[!is.na(bio$length_mm),]
bio <- rbind(bio, bads)
...
```{r abundcheck3, echo=F, results='hide'}
#### Present in biological information but not in abundance ####
# 2023
