...
tab <- abund %>% 
  dplyr::select(year, species_name, cpue) %>% 
...
data.scores$year <- rownames(data.scores)
...
# Assign k clustrers
km.out <- kmeans(data.scores[,1:2], centers = 2, nstart = 20)
...
data.scores$cluster <- factor(data.scores$cluster,
                              levels=c('herring',
                                       'silverside'))
data.scores$year <- as.numeric(data.scores$year)
...
test <- left_join(test, dplyr::select(data.scores, year, cluster))
...
ggplot() +
  geom_tile(data=test,
            aes(x=species_name, y=year,
                fill=log(cpue), col=cluster),
            width = 0.95, height = 0.98, 
            lwd=0.75) +
  scale_fill_viridis_c(na.value = 'transparent') +
  labs(fill = 'log(CPUE)', x='Species', y='') +
  theme(axis.text.x = element_text(angle = 90),
        legend.position = 'right')
