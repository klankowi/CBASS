...
#                                    'mud/gravel',
#                                    'mud'))
trips$site_name[trips$site_name == 'Stovers Point '] <- 'Stovers Point'
...
trips$loc_id <- paste0(trips$date, ' ', trips$site_name)
...
# Find duplicates
dups <- abund %>% 
  dplyr::select(date, site_name, species_name)
dups <- dups[duplicated(dups),]

abund$species_name[abund$species_name %in% c('blueback herring',
                                             'alewife', 
                                             'river herring')] <- 'river herring'

abund$species_name[abund$species_name %in% c('emerald shiner',
                                             'golden shiner',
                                             'shiner spp')] <- 'shiner spp'

abund$species_name[abund$species_name %in% c('hake',
                                             'hake spp',
                                             'red hake',
                                             'spotted hake',
                                             'white hake')] <- 'hake spp'
...
abund$species_name[abund$species_name %in% c('fourspine stickleback',
                                             ...
                                             'stickleback spp',
                                             'threespine stickleback')] <- 'stickleback spp'

abund$species_name[abund$species_name %in% c('sculpin',
                                             'grubby sculpin',
                                             'shorthorn sculpin',
                                             'slimy sculpin',
                                             'longhorn sculpin')] <- 'sculpin spp'

abund$species_name[abund$species_name %in% c('sturgeon',
                                             'sturgeon spp')] <- 'sturgeon spp'

abund$species_name[abund$species_name %in% c('smelt',
                                             'rainbow smelt')] <- 'rainbow smelt'

abund$species_name[abund$species_name %in% c('sandlance',
                                             'sand lance')] <- 'sandlance'

# Clean
abund <- abund %>% 
  mutate(date = as.Date(date, format = '%m/%d/%Y')) %>% 
  mutate(loc_id = paste0(date, ' ', site_name)) %>% 
  filter(loc_id %in% trips$loc_id) %>% 
  group_by(loc_id, date, site_name, site_id, bay_location, species_name) %>% 
...
dat <- merge(trips, 
             dplyr::select(abund, -site_id, -date, -bay_location, -site_name),
             by=c('loc_id'),
             all=T)
dat <- dat %>% 
  dplyr::select(-bay_location, -substrate, -set_time_local, -weather, -temp_degc,
                -do_mg.l, -salinity_ppt, -hermit_crabs, -shrimp, -notes, -month,
                -week, -day, -doy)
...
# Clean
dat <- dat %>% 
  pivot_longer(cols = colnames(dat)[6:52],
...
dat <- left_join(dat, tc, by='year')


# Yearly catch by species
yc <- dat %>% 
  group_by(year, species_name, hauls) %>% 
  summarise(catch = sum(catch)) %>% 
  mutate(cpue = catch / hauls) %>% 
  as.data.frame()

yc <- yc %>% 
  group_by(species_name) %>% 
  #filter(catch == min(catch)) %>% 
  ...
  arrange(cpue) %>% 
  #filter(cpue == min(cpue)) %>% 
  #filter(species_name == 'river herring') %>% 
  #filter(year == 2025 & cpue >0) %>% 
  as.data.frame()

yc.list <- split(yc, f=yc$species_name)
for(i in 1:length(yc.list)){
  for(j in 1:nrow(yc.list[[i]]))
  yc.list[[i]]$rank[j] <- j
}
yc <- do.call(rbind, yc.list)
rownames(yc) <- NULL
yc$rank[yc$rank == 1] <- 'Lowest CPUE'
yc$rank[yc$rank == 2] <- '2nd lowest CPUE'
yc$rank[yc$rank == 3] <- '3rd lowest CPUE'
yc$rank[yc$rank == 4] <- '4th lowest CPUE'
yc$rank[yc$rank == 6] <- '6th lowest CPUE'
yc$rank[yc$rank == 9] <- '3rd highest CPUE'

labs <- yc[yc$year == 2025,]
labs$x <- c(2019, 2017, 
            2019,
            2019, 2020, 2019, 
            2019,
...
pcatch <- ggplot(data=yc) +
  ...
  geom_text(data = labs,
            ...
            col='gray40') +
  facet_wrap(vars(species_name), scale='free_y') +
  labs(x='Year', y='CPUE (inds/hauls)')

ggsave(plot=pcatch,
       filename = here('State of the Bay/Selected Species CPUE.png'),
       width = 12, height = 8, units='in')

  
#Yearly catch in total
dat %>% 
    group_by(year, hauls) %>% 
    summarise(catch = sum(catch)) %>% 
    mutate(cpue = catch / hauls) %>% 
